...
/// with a 32-byte key, a 12-byte nonce, and an optional initial counter value. The <see
...
public sealed class ChaCha20StreamCipher
{
    ...
    /// <param name="nonce">The nonce (number used once) used to ensure unique encryption streams. The nonce must be 12 bytes in
    /// length.</param>
    ...
    /// <exception cref="ArgumentException">Thrown if <paramref name="key"/> is not 32 bytes in length, or if <paramref name="nonce"/> is not 12 bytes
    /// in length.</exception>
    ...
    public void Transform(ReadOnlySpan<byte> input, Span<byte> output)
    {
        ...
        if (_exhausted && input.Length != 0)
            throw new InvalidOperationException("The ChaCha20 counter has been exhausted.");

        ChaCha20.Encrypt(_key, _nonce, _counter, input, output);

        // Advance counter by number of 64-byte blocks
        ulong blocks = ((ulong)input.Length + 63) / 64;
        ulong nextCounter = (ulong)_counter + blocks;
        if (nextCounter > uint.MaxValue)
            _exhausted = true;
        else
            _counter = (uint)nextCounter;
    }
